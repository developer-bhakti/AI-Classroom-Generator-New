...
const ActivityIdeas = () => {
  const [formData, setFormData] = useState({ topic: "Community Helpers", className: "Class 2", subject: "English", duration: "30 mins", objective: "Encourage collaboration" });
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-panel">
        <Navbar title="Activity Ideas" />
        <div className="content-area grid-layout">
          <form className="panel-card" onSubmit={handleSubmit}>
            <h3>Plan a lively activity</h3>
            <label>Class</label>
            ...
            <button className="primary-btn full" type="submit">Generate activities</button>
          </form>

          <div className="panel-card output-card">
            {loading ? <p>Generating fresh activity ideas...</p> : result ? (
              <>
                <h3>{result.title}</h3>
                <p>{result.summary}</p>
            ...
            ) : <p>Bring energy into your classroom with exciting activities.</p>}
          </div>
        </div>
      </main>
    </div>
